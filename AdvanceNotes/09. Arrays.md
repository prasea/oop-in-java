# 📘 **Java Arrays — Clean Notes & Explanation**
...
## 📌 **5. Traversing Arrays (visiting all elements)**

There are two ways:

---

### ▶ **A. Using Normal For Loop (classic loop)**
...
# 📘 **Java Arrays — Practising 1D Array**
...
* Size is automatically determined → **5 elements**.
...
> ✔ In Java, you may put `[]` after the data type **or** after the variable name.
> ✔ Both are valid.
...
✔ Directly gives **elements**
✔ Cannot modify array values
✔ Cannot access index
...
* `[ I` → indicates **array of int**
* `@4617c264` → hash code / memory reference

### Why doesn’t Java print all elements?
...
### ✔ To display all elements → use **for loop** or **for-each loop**

### ✔ for-each cannot modify the actual array

### ✔ Classic for-loop can modify array elements

### ✔ Printing the array directly prints a **reference**, not values

---



# ✅ **1. Traversing (Scanning) an Array**
...
* If found → print index and stop the program
* If loop finishes and still not found → print "Not Found"
...
For the given array → **Second largest is 12**
...
# 📘 Lecture Notes: Array Rotation, Insertion & Deletion
...
* Understand shifting logic—the basis of all three operations.
...
Arrays cannot physically “delete” a slot. The index still exists, but we treat that position as overwritten by shifting elements.
...
| Insertion      | Elements from index→end | Right     | No                  |
| Deletion       | Elements from index→end | Left      | No                  |
...
* Old array becomes unreachable → garbage collector removes it.
...
| ----------------------- | -------------------------------------- | --------------------------------- |
| **Copy array**          | Loop from `0` → `length-1`             | Simple assignment                 |
| **Reverse-copy array**  | Use two index variables (`i--`, `j++`) | Left → Right and Right → Left     |
| **Increase array size** | Create new array, copy, reassign       | Arrays cannot be resized directly |

---

# **📘 Two-Dimensional Arrays (2D Arrays) in Java**
...
That means a 2D array is NOT one big block — it is:
...
## **1️⃣ Creating a 2D Array (Regular/Matrix)**
...
```
a → [ ref0 | ref1 | ref2 ]      // array of references
...
## **2️⃣ Creating & Initializing a 2D Array (Shortcut Method)**
...
## **3️⃣ Creating a 2D Array in Two Steps**
...
## **4️⃣ Accessing & Printing 2D Arrays Using Nested Loops**
...
## **5️⃣ Accessing a 2D Array Using For-Each Loop**
...
## **6️⃣ Jagged Arrays (Ragged Arrays)**
...
```java
a[0] = new int[2];   // row 1 → 2 columns
a[1] = new int[4];   // row 2 → 4 columns
a[2] = new int[3];   // row 3 → 3 columns
```
...
# **📘 Two-Dimensional Arrays (2D Arrays) in Java – Complete Explanation**
...
This creates a 5×5 matrix.

---

## **2️⃣ Initializing a 2D Array (Static Initialization)**
...
## **3️⃣ Declaring First, Creating Later**
...
## **4️⃣ Placement of Square Brackets**
...
```java
int e[5];    // ❌ ERROR — size cannot be given at declaration like C/C++
```

Java requires:

```java
int[] e = new int[5];  // ✔ correct
```

---

## **6️⃣ Printing a 2D Array Using Nested For-Loops**
...
## **7️⃣ Accessing a 2D Array Using For-Each Loop**
...
## **8️⃣ Printing the Array Reference**
...
## **9️⃣ Jagged Arrays (Ragged Arrays)**
...
a[0] = new int[5];         // row 0 → 5 elements
a[1] = new int[3];         // row 1 → 3 elements
a[2] = new int[8];         // row 2 → 8 elements
```

Jagged means:
Row 0 size ≠ Row 1 size ≠ Row 2 size

---

## **🔟 Printing a Jagged Array (Nested Loops)**
...
## **1️⃣1️⃣ Printing Jagged Array Using For-Each Loop**
...
# **2. Two-Dimensional Arrays (2D Arrays) Programs**
...
Matrices **A (m × n)** and **B (n × p)** can be multiplied **only if**:
