...
* Multiprogramming: Run multiple programs on one CPU → better CPU utilization.
* Multi-user: Multiple users share a high-performance computer.
* Multitasking: Single user runs multiple tasks.
* Multi-threading: Multiple threads inside a single program → advanced multitasking.
* CPU is underutilized in single-task programs → multi-threading maximizes efficiency.
...
1. **Animations**
   ...
   * Each ball can be controlled by its own thread → independent movement.
...
3. **Multimedia Applications**
   ...
   * Each instrument is controlled by a separate thread → simultaneous sound playback.

4. **Web Servers**

   * Example: Handling multiple users on a website.
   * Each user request handled by a **separate thread**.
   * Thread fetches user-specific data from database → **responsive web service**.
...
* CPU executes **alternatively**, OS ensures scheduling → illusion of simultaneity.
...
2. What makes threads “lightweight” compared to full processes?
...
1. **Control Flow in Single-Threaded Program**
   ...
   * Analogy: Reading a book with references to other pages – you go to a page, read it, then return to the main page.
...
3. **Need for Multiple Flows of Control**
   ...
   * Result: Two concurrent flows of control → `Hello` and `World` printed alternately.
...
* One flow of control → statements executed sequentially.
* Infinite loop blocks remaining code → **cannot achieve simultaneous execution**.

### Multi-Threaded Execution

* Convert methods or tasks into threads.
* Main thread continues execution **while child threads run independently**.
* CPU alternates execution between threads → users perceive **simultaneous execution**.
...
* CPU executes alternately → creates **illusion of parallel execution**.

---

## **Summary**

* Single-threaded programs have **one control flow** → sequential execution.
* Infinite loops in a single thread **block further statements**.
* Multi-threading introduces **multiple flows of control** → tasks can run concurrently.
...
# **📘Java Multi-Threading: Using Thread Class**
...
1. **Do not call `run()` directly** → it executes in main thread, not a separate thread.
2. **Always call `start()`** → it internally invokes `run()` and creates a new control flow.
3. **Thread scheduling** is handled by JVM → CPU alternates between threads giving **illusion of simultaneous execution**.
...
# **📘Java Multi-Threading: Using Runnable Interface**
...
2. **Running a Runnable**
   ...
   * Example analogy:
     ...
     * Start the thread → horse runs → cart logic executes.
...
* `My` implements Runnable → thread logic in `run()` method.
* `Thread t = new Thread(m)` → `Runnable` object attached to thread.
* `t.start()` begins execution of `run()` in a **separate thread**.
...
2. Modify loops to **print numbers 1–10** instead of infinite loops for testing.
3. Observe **mixed output**: “Hello” and “World” printed simultaneously.
...
* Two threads run **simultaneously**:
  ...
  * `MyThread` → prints "Hello"
  * Main thread → prints "World"
...
* **Real-world analogy:** One printer, multiple employees—only one uses it at a time, others wait their turn.
...
* **Key Notes:**
  ...
  * `wait()` → waiting state
  * `notify()` → moves thread from waiting to runnable
  * `sleep()` → timed waiting, resumes automatically
  * Resource access → may cause blocked state
...
1. **MS Word**
   ...
   * **Input thread** (typing) → High priority
   * **Spell-check thread** → High priority
   * **Auto-save thread** → Low priority
   * **Reason:** User input should not be delayed due to auto-saving.

2. **Web Browser (Chrome, IE)**

   * **Data fetching thread** → High priority
   * **Rendering thread** → Lower priority
...
* Java’s **JVM scheduler** manages threads; it may use the OS scheduler but **does not depend entirely on it**.
* Developers must **carefully assign priorities** to ensure smooth program execution.

---

# **📘Java Thread Class Overview**
...
* `getState()` → Returns thread state (`NEW`, `RUNNABLE`, `RUNNING`, `WAITING`, `BLOCKED`, `TERMINATED`).
* `getThreadGroup()` → Returns the thread’s group.

---

## **3. Setter Methods**

* `setName(String name)` → Change thread name.
* `setPriority(int priority)` → Change thread priority.
* `setDaemon(boolean)` → Set thread as **daemon** (background thread, low priority).
...
# **📘Java Thread Class: Constructors and Methods**
...
* **getId()** → Thread ID (assigned by JVM)
* **getName()** → Thread name
* **getPriority()** → Priority (1–10, default 5)
* **getState()** → Current state (NEW, RUNNABLE, BLOCKED, TERMINATED, etc.)
...
* Changes thread state from **NEW → RUNNABLE**.
...
# **📘Java Thread Methods: Daemon, Join, and Yield**
...
# **📘Java Multithreading: Synchronization Concepts**
...
3. If unavailable → thread waits in queue
...
Object (Monitor)
 ├── Data
 ├── Synchronized methods - read() & write()
 ├── Mutex for Lock & Unlock (implicit)
 └── Queue (implicit)
```
...
## D. Intuitive Analogy: Barber Shop 🪒
...
* If chair is free → get service
* If not → wait in queue
* Customer doesn’t manage queue or chair
...
* **Monitor ≠ OS feature**
  → Monitor is a **language-level (Java) abstraction**
...
✔ Matches standard OS & Java definitions
⚠ Simplification note: JVM still uses OS primitives internally, but **this is abstracted away** from the programmer (good abstraction, not an error)
...
* Threads only call methods — synchronization is automatic
...
3. Threads simply **call the method**:
   ...
   * If free → allowed
   * If busy → waits automatically
4. Two ways to implement:

   * **Synchronized block** → part of method
...
How **Java’s monitors** provide **easy, thread-safe access** to shared objects.

---


# **📘Java Monitors Example – Synchronization**
...
* **Thread Classes:**
  ...
  * `MyThread1` → prints `"Hello World"`
  * `MyThread2` → prints `"Welcome"`
...
4. Two approaches:
   ...
   * **Synchronized block** → part of method
   * **Synchronized method** → entire method
...
* **Operations:**
  ...
  * `checkBalance(customerName)` → prints a message that the customer is checking balance.
  * `withdraw(customerName, amount)` → prints a message that the customer is withdrawing a specific amount.
...
* **Methods:**
  ...
  * `checkBalance(String name)` → prints `name + " checking balance"`, with sleep to simulate delay.
  * `withdraw(String name, int amount)` → prints `name + " withdrawing " + amount`, with sleep.
* **Synchronization:**
...
* **Properties:**
  ...
  * `ATM atm` → Reference to the shared ATM object
...
* One shared object (`ATM`) → acts as a monitor.
* Multiple threads (`Customer`) → reference same ATM object.
* `synchronized` keyword → ensures only one thread uses ATM at a time.
* Demonstrates **real-life synchronization** scenario using Java multithreading.

---

# **📘Synchronization: Interthread Communication & Race Conditions**
...
* Producer writes → Consumer waits in Blocked Queue
* Producer finishes → Consumer reads → Producer waits in Blocked Queue
...
* **Race condition** occurs if:
  ...
  * Access order is not controlled → may violate intended sequence.

---

### **Illustration**

* Producer writes data → notifies all waiting threads.
* Any thread may wake up → **unpredictable access** order.
* Example:

  * Producer should wait until **all consumers have consumed**.
  * If producer wakes up too early → violates expected sequence.
...
* Use a **counter variable** (`count`) to track turns:
  ...
  * `count = 0` → Producer’s turn
  * `count = 1,2,3` → Consumer 1,2,3 turn
* Steps:

  1. Producer produces → sets `count = 1`
  2. Consumer 1 consumes → `count = 2`
  3. Consumer 2 consumes → `count = 3`
  4. Consumer 3 consumes → resets `count = 0` → back to Producer
* Ensures **strict order of execution** → avoids race conditions.
...
3. **notify() and wait():**
   ...
   * `wait()` → thread waits for its turn.
...
---
# **📘Producer-Consumer Interthread Communication Example**
...
# **📘Teacher-Student-Whiteboard Problem (Multi-threading)**
...
2. **Students read:** Wait if `count == 0` (teacher hasn’t written yet).
