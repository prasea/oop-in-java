...
* **AWT (Abstract Window Toolkit)** – First GUI framework (Java 1)
* **Swing** – Improved GUI framework (Java 2)
* **JavaFX** – Most modern and advanced GUI framework

Swing sits between AWT and JavaFX and is designed to overcome AWT’s limitations while keeping GUI development relatively simple.
...
* On Windows → Windows-style components
* On macOS → macOS-style components
...
### B. What “Abstract” Means in AWT

The word **Abstract** in *Abstract Window Toolkit* does **not** mean Java’s `abstract` keyword.
...
AWT does **not define how components look** — the OS does.


### C. Swing’s Platform Independence Explained (Analogy)

**AWT analogy**
A person travels to different countries and wears **local clothes** in each place.
→ Appearance changes depending on location.
...
* AWT → Uses OS look and feel
* Swing → Uses Java-defined look and feel
...
* **AWT** was Java’s first GUI framework but is **platform dependent**
...
```
* `JFrame` is Swing’s main window container
...
* Much simpler than AWT’s `WindowListener`
...
* **`JFormattedTextField`** – input restricted to a specific format (date, number, currency, etc.)
...
* `15` → number of columns (controls width)
...
## `JFormattedTextField` – Why It Matters
...
`new Date()` sets today’s date
Value `0` → 1 January 1970 (Java epoch)
...
* `NumberFormatter` → enforces those rules
* `JFormattedTextField` → applies formatter
...
## F. `JTextArea` – Multi-line Text Input
...
* Both checkboxes together → **Bold + Italic**
* Radio buttons:
...
* In **AWT** → usually `ItemListener`
* In **Swing** → can use **ActionListener**
...
✔ Swing allows **String-based switch cases**, which makes code cleaner.
...
✔ Advantage: same method, different appearance using parameters
...
✔ This lets you place the title **inside or outside the border**
...
✔ Useful for advanced UI styling
✔ Often asked as a theory question
...
* Menu structures
* Any parent–child structured data
...
* `JTree` → to display hierarchical data
* `JLabel` → to show the selected node information
* `JScrollPane` → to add scrolling
* `JFrame` → main window
* `DefaultMutableTreeNode` → tree nodes
* `File` (java.io) → read directory contents
...
Trees can grow large → scrolling is required.
...
* Tree (inside scroll pane) → `CENTER`
* Label → `SOUTH`
...
⚠ This is **tree path**, not OS file system path
...
* `JLayeredPane` adds a **third dimension (z-axis)**
  → components can be placed **above or below** each other

Think of layers as **transparent sheets stacked from screen → towards you**.
...
* You need multiple “screens” inside one frame
...
* `JLayeredPane` → manages layers
* `JLabel` → colored rectangles (panels)
* `JRadioButton` → layer selection
* `ButtonGroup` → mutual exclusivity
* `JPanel` → holds radio buttons
* `MouseMotionListener` → track mouse movement
* `ActionListener` → handle radio button clicks
...
* Higher layer number → appears **on top**
* Same layer → overlap depends on addition order
...
* Red selected → black label above red
* Green selected → black label above green
* Blue selected → black label above blue
...
* Radio button panel → `BorderLayout.NORTH`
* Layered pane → `BorderLayout.CENTER`

⚠ `JLayeredPane` usually uses **absolute positioning**, so `setBounds()` is required.


## 12. Important Learning Points (⭐ Exam Focus)

* `JLayeredPane` supports overlapping components
* Components are arranged using **layer numbers**
* Higher layer → appears in front
...
# **JInternalFrame in Java Swing – Structured Learning Notes**
...
* Internal menu (File → Save)
* Fixed starting position
* Fully controllable window behavior

📌 **Constructor Flags Used**
...
* **JFrame** → Desktop monitor
* **JDesktopPane** → Desktop workspace
* **JInternalFrame** → Application windows (Word documents)
...
| Why JScrollPane needed?           | JTextArea alone doesn’t support scrolling                             |
...
# **JSplitPane and JTabbedPane in Java Swing – Structured Learning Notes**
...
## **Model–View Concept (Important Theory)**

* **Model** → Data (array of color strings)
* **View** → UI component (`JList`)
...
* Ideal for **master–detail layouts**
* Common in file managers and admin tools



# **JTabbedPane – Alternative Layout**
...
## **Conversion: Split Pane → Tabbed Pane**
...
| Why setOpaque(true)?        | JLabel won’t show background color otherwise  |
...
# **Java Swing – JSlider and JProgressBar**


1. **JSlider** – a control for selecting values within a range (similar to a scroll bar).
2. **JProgressBar** – a visual component showing progress or task completion.
...
* Represents **a value within a range** (min → max)
* Can be **horizontal or vertical**
* Supports **major ticks** (e.g., 0, 10, 20 …)
...
1. `JSlider()` – default horizontal slider, 0–100
2. `JSlider(min, max)` – range with default value
3. `JSlider(min, max, initialValue)` – range with initial value
4. `JSlider(orientation, min, max, initialValue)` – allows vertical/horizontal orientation
...
* **BorderLayout**:
  ...
  * **North** → Slider
  * **Center** → Panel for drawing circle
  * **South** → Panel containing progress bar
...
* Both can interact dynamically (slider value → circle size & progress bar)
* Essential for interactive Swing applications (games, tools, installers)


# **Java Swing – JComboBox, JList, and JSpinner**
...
* **JComboBox** → Single selection visible, dropdown only
* **JList** → Multiple selection possible, scrollable
* **JSpinner** → Increment/decrement numeric or list values
* Swing uses **MVC pattern**:

  * **Model** → Data (array or SpinnerModel)
  * **View** → Component (JComboBox, JList, JSpinner)
...
# **Java Swing – JMenuBar, JMenuItem, and JToolBar**
...
> **Note:** Image buttons don’t have a visible label, so you can set an **action command** manually if needed:
...
# **Java Swing – ActionListener, JFileChooser, and JColorChooser**
...
✅ **Resulting Behavior in Your App**
