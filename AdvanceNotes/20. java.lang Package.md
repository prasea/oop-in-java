...
### **1. What’s inside the Object Class?**
...
* Similar to destructor in C++, but unreliable → rarely used today.
...
* Returns a unique integer for each object. No two object will have same hash code. 
* Generated using the object’s internal memory address (indirectly).
...
* **All are final** → cannot be overridden.
...
public class LangDemo {
  public static void main(String[] args) {
    Object o1 = new Object();
    System.out.println(o1);       // Implicitly calls toString()
...
Java guarantees that **no two different objects share the same hashCode by default**. It generates utilizing the internal address of an object
...
## **4. Overriding Object Class Methods**
...
class MyObject { 
  ...
  public int hashCode() {
    return 100;   // every object gets same hash — breaks hashing
  }
}
public class LangDemo {
  public static void main(String[] args) {
...
## **Application (Where This Knowledge Is Used)**
...
```java
Integer i = 10;
float f = i.floatValue();   // int → float
```

---

## **6. Creating Wrapper Class Objects**
...
Byte error = Byte.valueOf(15);     // Error : 15 is Integer Literal
byte bb = 15;
...
**Boxing** = Converting a primitive → Wrapper object.
...
```java
int m = 10;
Integer n = m;         // Auto-boxing equivalent to Integer n = Integer.valueOf(m);
```
...
```java
int m = 10;
Integer h = m;  // Auto-boxing equivalent to Integer h = Ingeger.valueOf(m)
int y = h;      // Auto-unboxing equivalent to int y = h.intValue();
```

A primitive is assigned to object is Auto-Boxing.
Object is assigned to primitive is Auto-Unboxing.  

---

## **Boxing and Unboxing Summary**
...
# **📘Integer Wrapper Class — Complete Notes**

The **Integer** class is one of Java’s most commonly used wrapper classes. It wraps a primitive `int` inside an object and provides many useful methods.

---

## **1. Creating Integer Objects**

### **Primitive → Integer (Auto-boxing)**

```java
int m1 = 15;
Integer m2 = m1;   // auto-boxing Integer.valueOf(m1)
```
...
* `Object.equals()` → checks if **references are same**.
* `Integer.equals()` → overridden to check **numeric value**.
...
int m1 = 15;
Integer m2 = m1; 
System.out.println(m2.equals(m1)); // true (values both 15)
```
...
Integer m3 = 15;
System.out.println(m2.equals(m1)); // true (values both 15)
System.out.println(m2.equals(m3)); // true (values both 15)
```
...
```java
Integer m3 = Integer.valueOf("1010", 2);   // binary → 10
System.out.println(m3);
Integer m3 = Integer.valueOf("A7", 16);    // hex → 167
System.out.println(m3);
```
...
# **📘Wrapper Classes: Float, Character, Boolean — Complete Notes**
...
Float’s equals() behaves like Integer’s equals() → compares numerical value.
...
NaN occurs when result is **undefined**. Math.sqrt() returns double value. Hence type-casted to float. 
...
###  Wrong/unreliable method to check NaN
...
Two NaN are never equal. NaN **cannot** be compared using `==` or `equals()`. If one is Number and another is NaN, use `!=` for comparision.

### Rule:

➡️ **Always use `isNaN()`** to check if a float/double is not a number.
...
| Convert String → boolean | parseBoolean                      |
...
* Use **isNaN()** → NEVER compare NaN using == or equals().
* Float division by zero → no exception → results in ±∞ or NaN.
...
* **Capacity** → size of the internal character array.
...
* No synchronization → better speed.
...
# **📘Java Math Class — Complete Notes**
...
**Bottom line:**
Use **Math** class in your programs.
StrictMath is rarely used directly.
...
```
When you try to decrementExact on Integer.MIN_VALUE, you get ArithmeticException, Because:
...
Returns **exponent** of the floating-point representation (mantissa–exponent form).
...
### **12. `atan()` — Inverse Tan (tan⁻¹)**

Returns result in radians:

```java
double r = Math.atan(1); // π/4 radians
...
* Provided methods include:
  ...
  * `ordinal()` → index (0-based) according to declaration order
  * `valueOf(String)` → returns enum constant matching a string
  * `toString()`
...
DEPT dep = DEPT.valueOf("it"); // ❌ IllegalArgumentException
```
...
The constructor runs (Not when assigning `DEPT d = DEPT.ECE;`)
...
* Example:
  ...
  * `DEPT.CS` → object with `head="John"`, `location="Block A"`
  * `DEPT.IT` → different object with its own fields
...
## **9. Summary of Key Learnings**
 ...
 Can use constructors (private/default)

 Can add fields and methods

 Constructor runs at **class loading**, not object creation
