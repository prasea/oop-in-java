...
* Java-provided **toolkit/framework** for GUI development
* Used to build **window-based (GUI) applications**
* Components adapt their appearance to the **underlying OS**

### Abstract vs Concrete

* **Concrete toolkit** → fixed look and feel
* **Abstract toolkit** → appearance depends on OS
* AWT components look:

  * Like Windows apps on Windows
  * Like macOS apps on macOS

### Component

* The **basic building block** of all GUI elements
* Parent class for buttons, text fields, labels, etc.

### Container

* A component that can **hold other components**
* Example: Frame, Panel, Applet
...
* Represents a window with borders
* Includes:
...
### Frame-Based Application

* A **desktop GUI application**
* Uses `Frame` class
* Runs directly on the operating system (like Notepad)

### Layout Manager

* Controls **size and positioning** of components
* Default layout of Frame: `BorderLayout`
* `FlowLayout` allows components to appear in natural size

### Important AWT Methods
...
Clicking does **not close the application**

* This requires **event handling**
* Will be covered later

For now:

* Stop program forcefully
...
###  Problem: Button Covers Entire Frame
...
class MyFrame extends Frame {
  ...
  public MyFrame() {
    super("My App"); // Calls Frame constructor
...
Java provides **event classes** inside:
...
* `super("Button Demo")` sets frame title
...
* What event is generated when a button is clicked?
* Which listener handles ActionEvent?
* What is a callback method?
* Why is listener registration required?
* Difference between Button and Label methods?
...
```
Label text → "Nothing Is Selected"
```
...
Instead of using `ItemEvent`, directly ask the checkbox:
...
## 9. Core Logic (Checkbox Handling)
...
## 1 Summary (Exam-Focused)

### Key Takeaways

* Checkbox & Radio Button use **same class**
* `CheckboxGroup` → converts checkbox into radio button
* `ItemListener` handles selection changes
* `getState()` is the most important method
* Label updates dynamically based on selection



## 1One-Line Exam Answers

* **Checkbox allows:** Multiple selections
* **Radio button allows:** Single selection
* **Listener used:** `ItemListener`
* **Event method:** `itemStateChanged()`
* **Radio button group class:** `CheckboxGroup`
...
public void textValueChanged(TextEvent te) {
    labelsetText(tf.getText()); // update Label1 while typing
}
```
...
public void actionPerformed(ActionEvent ae) {
  labelsetText(tf.getText()); // update Label2 on Enter
}
```
...
## 9. Exam-Friendly Notes

* **TextListener:** `textValueChanged(TextEvent e)` → typing
* **ActionListener:** `actionPerformed(ActionEvent e)` → Enter key
* **TextField Methods:** `getText()`, `setText()`, `setEchoChar()`
* **Inner class:** Avoid constructor reference warning (`leaking this`)
 ...
 **Layout Management**: Use **FlowLayout** to prevent TextArea from occupying the full frame.
 **TextArea behavior**: Automatically shows horizontal and vertical scrollbars if text exceeds bounds.
 **Text Selection**: Use `getSelectedText()` for copy/paste or formatting operations.
 **Appending vs Inserting**:

   * `append()` → Always adds at end
   * `insert()` → Adds at cursor, provides more flexibility
Explore other methods like `replaceRange()`, `select()`, `setCaretPosition()` for advanced text editing.



## Summary

* **TextArea** → Multi-line editable text component.
* **Key methods:** `getText()`, `getSelectedText()`, `append()`, `insert()`, `getCaretPosition()`.
* **Event Handling:** Attach `ActionListener` to a button to manipulate TextArea content.
* Useful for **Notepad-like applications** and **text editors**.
...
* **Choice**: A **dropdown box** component.
  ...
  * Similar to radio buttons but used for many options.
...
* `List L` → List box for days of the week
* `Choice C` → Dropdown for months
* `TextArea TA` → Display selected items

**Constructor Example**:

```java
L = new List(4, true);   // 4 visible rows, multiple selection allowed
C = new Choice();         // Single selection dropdown
TA = new TextArea(20, 30); // 20 rows, 30 columns
```
...
public void itemStateChanged(ItemEvent ie) {
  if (ie.getSource() == L) {
    TA.setText(L.getSelectedItem()); // ListBox selection
  } else if (ie.getSource() == C) {
    TA.setText(C.getSelectedItem()); // Choice selection
  }
}
```
...
* **Goal**: Display **all selected items** from a multi-select ListBox.
...
* **Behavior**: Components arranged in a **row**, wrap to next line when needed.
...
fl.setAlignment(FlowLayout.RIGHT);      // Right-align components
fl.setHgap(20);                         // Horizontal gap
fl.setVgap(10);                         // Vertical gap
setLayout(fl);                           // Apply to Frame
```
...
* Default layout for **Frame**.
* **Each region can hold only one component**, but you can nest containers like **Panels** to hold multiple components.
...
If you want, I can also create a **visual diagram showing FlowLayout, BorderLayout, and GridLayout examples** with buttons arranged in frames. This is very useful for **quick revision and exams**.

Do you want me to make that diagram?


Here’s a **structured, concise summary** from your video on **GridLayout and GridBagLayout in Java AWT**:
...
class MyFrame extends Frame implements KeyListener {
  ...
  MyFrame() {
    ...
    lsetBounds(20, 20, 100, 20);
    lsetBounds(20, 50, 100, 20);
    lsetBounds(20, 80, 100, 20);
    lsetBounds(20, 110, 200, 20);
  ...
  public void keyPressed(KeyEvent k) {
    ...
    lsetText("");
    lsetText("");
    lsetText(new java.util.Date(k.getWhen()) + "");
  }

  public void keyReleased(KeyEvent k) {
    lsetText("KeyReleased");
    lsetText("");
    lsetText("");
    lsetText("");
  }
