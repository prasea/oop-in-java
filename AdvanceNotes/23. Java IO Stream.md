# Java I/O (Input–Output) — Conceptual Foundations
...
The program **cannot directly touch these resources**—it must communicate via I/O mechanisms.
...
* Fast sender → slow receiver → data loss
* Slow sender → fast receiver → wasted time
...
❌ Data is **NOT sent as a whole object**
...
Even character streams internally work with bytes—Java abstracts this for Unicode support.
...
Used when you **don’t want to fill the entire array** from index `0`.
...
# **📘java.io Class - Java Stream, Reader, and Writer Classes**
...
* `InputStream` & `OutputStream` → all **byte streams**
* `Reader` & `Writer` → all **character streams**
...
### ⭐ Most Useful InputStreams (Practically)
...
3. Replace:
   ...
   * `InputStream → Reader`
   * `OutputStream → Writer`
4. Buffered, File, Data, Object, Piped → same idea everywhere

If you remember **5–6 key classes**, the rest become predictable.
...
# **📘Java File I/O: FileOutputStream**
...
## **2️⃣ Writing to a File Using FileOutputStream**
...
* `FileOutputStream.write()` has **three forms**:
  ...
  1. `write(int b)` → writes one byte.
  2. `write(byte[] b)` → writes entire byte array.
  3. `write(byte[] b, int offset, int length)` → writes part of the byte array.
...
* Convert **Strings → bytes** using `.getBytes()`.
...
# **📘Java File I/O: FileInputStream & FileReader**
...
* `fis.available()` → returns number of bytes available in the file
* `fis.read(byte[] b)` → reads all bytes into the array.
* Convert byte array → String → print.
...
* `fis.read()` → reads **one byte** and returns its ASCII value.
...
* Only minor changes: `FileInputStream → FileReader`
...
1. **FileInputStream vs FileReader**
   ...
   * `FileReader` → reads **characters** (Unicode-friendly).

2. **Reading Options**

   * **All bytes at once** → `read(byte[] b)`
   * **Byte by byte** → `read()`
...
* Using FileReader/FileWriter → same logic as FileInputStream/FileOutputStream but works with **characters**.
...
# **📘Java File I/O Challenge - Copy a File**
...
* ASCII conversion: `A-Z` (65–90) → add 32 → `a-z` (97–122).
...
1. **Task 1** → Copy file and convert uppercase letters to lowercase.
2. **Task 2** → Merge two files sequentially using `SequenceInputStream`.
...
✔ Efficient when the entire data can fit into memory.
...
⚠️ Instructor note: Mark/reset is better demonstrated using `BufferedInputStream`
...
🔹 Without typecasting → ASCII values
🔹 With typecasting → characters
...
✔ Same logic
✔ Same `read()` method
✔ Only difference: character-based data
...
* Producer → pushes water (data) into the pipe
* Consumer → reads water from the pipe
...
## **3️⃣ Main Method: Connecting Pipes**
...
## **1️⃣ What are Random Access Files?**
...
## **2️⃣ Java Class for Random Access**
...
* Provides key methods:
  ...
  * `seek(long position)` → move pointer to a specific byte
  * `getFilePointer()` → get current pointer position
...
Pointer → initially at 0

Read 3 bytes:
Pointer → moves to 3 (D)

Write 'd':
File → A B C d E F G H I J
Pointer → moves to 4 (E)

Skip 3 bytes:
Pointer → moves to 7 (I)
...
**💡 Summary:**
Random Access Files are powerful when you need **non-sequential reading/writing** and know the file’s internal structure. They are ideal for **databases, indexed files, or any file requiring frequent updates at specific positions**.

---

# **📘File Class**

## **1️⃣ What is the `File` class in Java?**
...
* `list()` → returns **names** of files/folders.
* `listFiles()` → returns **File objects**, so you can get more info like path, type, etc.

---

## **5️⃣ Example: Handling Read-Only Files**
...
* When a file is **read-only**, writing to it causes a `FileNotFoundException` with “Access is denied”.
...
**2️⃣ First Solution: Store Object as Text (Strings)**
...
**3️⃣ Problems with the First Solution**
...
## **2️⃣ Writing the Object to a File**
...
✅ **Result:** The file contains three lines: `"10"`, `"John"`, `"CSE"`.
...
## **3️⃣ Reading the Object from a File**
...
## **4️⃣ Key Points from the Demonstration**
...
* **First solution:** Used `PrintStream` → stored everything as **strings**, even integers.
...
## **2️⃣ Second Solution: Using Data Streams**
...
## **3️⃣ Key Advantage Over First Solution**
...
* **Desired solution:** Store the **entire object** at once and retrieve it — without accessing internal members individually.
...
# **📘Data Streams : DataInputStream & DataOutputStream DEMO**
---
...
* Streams used:
  ...
  * **DataInputStream** → for reading data

---

## **2️⃣ Writing Data with DataOutputStream**

1. **Create `FileOutputStream`** → connects to a file.

```java
FileOutputStream fos = new FileOutputStream("MyJava\\Student2.txt");
```

2. **Create `DataOutputStream`** → attach to `FileOutputStream`.
...
* Data is stored in **binary format** → not human-readable. Hence if you open [Student2.txt], it has weird symbold
* Text files are readable because data is converted to strings; binary files are stored in raw bytes.

---

## **3️⃣ Reading Data with DataInputStream**

1. **Create `FileInputStream`** → connects to the same file:

```java
FileInputStream fis = new FileInputStream("MyJava\\Student2.txt");
```

2. **Create `DataInputStream`** → attach to `FileInputStream`:
...
* **Binary vs Text Files:**
  * Text file → human-readable, stored as strings.
  * Binary file → stored in raw bytes, may not be readable in a text editor.
...
* That’s where **Serialization** comes in → allows storing the **whole object**.
...
## **2️⃣ How to Serialize an Object**
...
## **3️⃣ How to Deserialize an Object**
...
## **4️⃣ Important Rules for Serialization**
...
# **📘Object Streams & Serialisation DEMO**
...
* Key advantage: You don’t need to manually write/read each property; the **entire object** is handled automatically.

---

## **2️⃣ The Student Class for Serialization**
...
1. **`implements Serializable`** → required for object serialization. It’s a marker interface; no methods to override.
2. **Transient & Static members** → not serialized.
3. **Default constructor** → good practice, especially for inheritance.

---

## **3️⃣ Serializing an Object**
...
1. **Transient & Static Members:**
   ...
   * Not serialized.
   * Example: `t` (transient) and `staticValue` → values are not saved in file. It's value is what we set globally NOT in constructor. 
...
## **1️⃣ Task 1: Store and Retrieve Float Numbers**
...
## **2️⃣ Task 2: Serialize and Retrieve Customer Objects**

### **Problem**

* Define a `Customer` class with:

  * `customerID` (auto-generated: C1, C2, …)
  * `name`
  * `phoneNumber`
