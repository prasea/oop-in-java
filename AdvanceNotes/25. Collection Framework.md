...
## **2️⃣ Sources of Data for a Java Program**
...
## **5️⃣ Arrays: Basic Data Structure**
...
## **2️⃣ Why Arrays Are Not Always Suitable**
...
## **3️⃣ Collection Framework to the Rescue**
...
## **4️⃣ Common Operations on Collections**
...
Java provides Collection Framework in the form of interfaces and classes organized in the form of hierarchy. 

---

## **1️⃣ Introduction to Collection Framework**
...
## **2️⃣ List Interface (Ordered Collection)**
It extends Collection interface. So List is a Collection. 
...
## **3️⃣ Set Interface (Unordered, Unique Collection)**
It extends Collection interface. So Set is a Collection. 

* **Definition:** Unordered collection of **distinct elements** (no duplicates)

* **Features:**

  * Duplicates not allowed. If already object is there, you cannot insert it again.
...
## **4️⃣ Queue Interface (FIFO Collection)**

* **Definition:** Elements follow **First-In-First-Out (FIFO)** principle
* We can insert an element on collection from one end and delete from the other end.
...
## **6️⃣ Map Interface (Key-Value Pair Storage)**
It is separate hierarchy from Collection.  It is the implementation of hasing technique. 
...
* **Legacy Classes:** Vector, Stack, Hashtable — old versions, rarely used now.
...
## **1️⃣ Collection Interface Overview**
...
## **2️⃣ Key Methods of Collection Interface**
```java
...
| ----------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------- |
| `add(E e)`                          | Adds a single element to the collection                  | Add `G` to `[A, B, C, D, E]` → `[A, B, C, D, E, G]`            |
...
## **3️⃣ Notes on Iterator**

Collections do not use indexes like arrays
...
* **Methods:**
  ...
  * `remove()` – optionally remove the current element

* **Analogy:** `for (int i = 0; i < array.length; i++)` → `Iterator` does the same for collections without indexes
...
# **📘Understanding List, Set Interface**

List represent ordered collection of elements i.e. indexing is there and there may be duplicates too.
---
...
**Important Methods:**
It has access to all methods that we discussed in Collection interface. On top of that there are some unique methods too. 
...
| ---------------------------------------------- | -------------------------------------------------------------------- | --------------------------------- |
| `add(E e)`                                     | Adds an element at the end                                           | `[A,B,C] → add(D) → [A,B,C,D]`    |
| `add(int index, E element)`                    | Inserts element at a specific index                                  | Insert `K` at index 2             |
| `addAll(Collection<? extends E> c)`            | Adds all elements from another collection at the end                 | `[A,B] addAll([C,D]) → [A,B,C,D]` |
...
| `get(int index)`                               | Returns element at given index                                       | `get(2) → C`                      |
| `set(int index, E element)`                    | Replaces element at given index                                      | Replace index 3 with `K`          |
| `subList(int fromIndex, int toIndex)`          | Returns part of the list (view)                                      | `subList(1,4)` → `[B,C,D]`        |
| `indexOf(Object o)`                            | Returns first occurrence index of an object                          | `indexOf(C) → 2`                  |
...
* `add` vs `set`: `add` inserts new element and shifts old elements but `set` replaces old element at that index
* `listIterator` is bidirectional, unlike `iterator` which allowed to access the element only in forward direction.

---

## **2️⃣ Set Interface**

It doesn't have extra method. It extends Collection interface. So whatever methods are on Collection, the same is available on Set interface. Set is unordered collection of objects and unique collection of objects. There are distinct objects. 
...
* `poll` vs `remove`: Same action, different empty queue behavior. If the queue is empty then `poll()` will return null. If a queue is empty, then remove() throws NoSuchElementException
* `peek` vs `element`: Same action, different empty queue behavior. If the queue is empty then `peek()` will return null. If a queue is empty, then element() throws NoSuchElementException
...
## **1️⃣ Using Java Documentation**

* Java provides **official documentation** for all packages, classes, interfaces, and methods.
* You can search for `Java <version> <package>` (e.g., `Java 13 util`) to get Oracle’s documentation.
...
* **Extends:** `Collection` → inherits all Collection methods.
...
## **7️⃣ Tips for Learning Collections**
...
* **Initial Capacity**: 20 → array list can hold at least 20 elements initially, grows automatically as needed.
...
* **removeAll(Collection c)** → Remove all elements present in another collection.

---

## **5️⃣ Checking Elements**
...
## **6️⃣ Iterating through ArrayList**
...
* Java’s `LinkedList` class supports all List operations and additional methods from **Deque interface**.
...
* `addFirst(element)` / `offerFirst(element)` → Add to the front.
* `addLast(element)` / `offerLast(element)` → Add to the rear.
* **Difference**:

  * `addFirst/addLast` throw exceptions if insertion fails.
  * `offerFirst/offerLast` return `null` instead of throwing an exception (safer).
...
* `getFirst()` / `peekFirst()` → View first element.
* `getLast()` / `peekLast()` → View last element.
...
Instead of using **Stack**, use **ArrayDeque**
Instead of using **LinkedList** for Queue, use **ArrayDeque** 
...
PriorityQueue represents a data structure called Heap and it is implemented using Array.

## **1. What is a PriorityQueue?**

* A **PriorityQueue** is a data structure where elements are stored **based on priority**.
* **Priority is determined by the element’s value** (or a custom comparator).
...
* **Insertion:** Element is added at the next available position, then “bubbled up” to maintain heap property.
...
public class PriorityDemo {
  public static void main(String[] args) {
    ...
    Iterator<Integer> li = pq.iterator();
    while(li.hasNext()) {
...
public class PriorityDemo {
  public static void main(String[] args) {
    PriorityQueue<Integer> pqMax = new PriorityQueue<>(new MyCom());
    pqMax.add(20);
    pqMax.add(10);
    pqMax.add(30);
    pqMax.add(5);
...
```
Insert 15 in table of size 10: 15 % 10 = 5 → store at index 5
Insert 28: 28 % 10 = 8 → store at index 8
Insert 42: 42 % 10 = 2 → store at index 2
```
...
```
Insert 35: 35 % 10 = 5 → index 5 is occupied by 15 → check next index → insert 35 at index 6
```

* **Search:** Use hash function to find index element to search, check sequentially until value is found or a **blank bucket** is reached.
* **Important:** Table must have **some blank spaces** to terminate search. It is called Loading factor

  * Ideal fill: 50% keys, 50% blanks
  * Acceptable fill: ≤ 75% keys → blank spaces ensure search can terminate
...
```
Insert 15 → apply hash function → index 5 → store in bucket
Insert 35 → apply hash function → index 5 → store in linked list at bucket 5
```

* **Search:** Go to bucket → traverse linked list to find key.
...
* **LinkedHashMap** → preserves insertion order with key-value pairs
* **Hashtable** → legacy synchronized key-value store
* **Properties** → key-value pairs, often for configuration
...
* Two main collision strategies:
  ...
  * **Open Addressing** → linear probing in next available space
...
* For storing **custom objects**, TreeSet must know **how to compare them** to maintain sorted order.
...
* Goal: Define which point is **smaller** or **greater**:
  ...
  2. If `x` is equal on both points, compare **y-coordinate**.
...
2. Supports operations like:
   ...
   * `ceilingEntry(key)`: Returns entry with **key ≥ given key**.
   * `floorEntry(key)`: Returns entry with **key ≤ given key**.
...
2. **Access Order**:
   ...
   * Supports **LRU (Least Recently Used)** behavior — commonly used in **cache implementation**.
...
* **Access Order / LRU Cache**:
  ...
  * Insert 11th key → **least recently used key is removed**
...
* Maintains **insertion order**: 1 → 2 → 3 → 4 → 5
...
* LinkedHashMap **automatically resizes**, insertion order is preserved: 1 → 2 → 3 → 4 → 5 → 6 → 9 → 8
...
* **true** → keys are ordered by **access**, not insertion.
...
* Maintains **insertion order**: `A → C → E → K → B → G`.
...
| ------------------ | ---------------------------------------- | ------------------------ |
| Maintains order    | ❌ No                                     | Yes, insertion order   |
| Stores duplicates  | ❌ No                                     | ❌ No                     |
...
* Use **HashSet** if you only need uniqueness and don’t care about order.
...
* **`compute(key, BiFunction)`** → compute a new value for an existing key:

```java
ht.compute(2, (k, v) -> v + "Z"); // updates B → BZ
```

* **`computeIfAbsent(key, Function)`** → compute value only if key is absent:

```java
ht.computeIfAbsent(7, k -> "Z" + k); // inserts 7 → Z7
```
...
5. Useful **compute** methods allow mapping **key → value** via functions.
...
* **Set a bit:** `bitSet.set(index)` → makes the bit at `index` **true**.
* **Clear a bit:** `bitSet.clear(index)` → makes the bit at `index` **false**.
* **Flip a bit:** `bitSet.flip(index)` → toggles the bit at `index`.
* **Get bit:** `bitSet.get(index)` → returns `true` or `false`.
...
* `equals(array1, array2)` → returns `true` if arrays have same elements.
* `deepEquals(array1, array2)` → used for **arrays of objects** (nested arrays).
...
int[] a = {2, 4, 6, 8, 1, 3, 5};
int[] d = Arrays.copyOf(a, a.length); // copy all elements of array a
```
...
* `Arrays.parallelSort(array)` → uses **multi-threaded merge sort**, useful for large arrays.
...
# **📘Java Manual-Driven Account Program**
