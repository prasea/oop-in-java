...
* **Microsoft SQL Server** – Enterprise database by Microsoft
* **SQLite** – Lightweight, free database (used for learning)
...
* Example: City is unknown → City = NULL
...
* Students table → Roll Number (Primary Key)
* Department table → Department Number (Primary Key)
...
1. Find student’s Department Number
...
* Department Number in Student table → Foreign Key
* Department Number in Department table → Primary Key
...
* If Department 15 does not exist → Student with Dept No 15 cannot be inserted
...
* Relationships between tables are built using **primary key–foreign key**
...
The database used is **SQLite**, and the database file is `Univ.db`.
...
* `RENAME` – rename a table (rarely used)
...
### 4. DCL – Data Control Language (Optional)
...
* Department Table → Primary Key: `deptno`
* Student Table → Primary Key: `rollno`
* Student Table → Foreign Key: `deptno`
...
* Columns:
  ...
  * `name` → TEXT
  * `city` → TEXT
  * `deptno` → INTEGER (FOREIGN KEY → Department.deptno)
...
## J. Key Learning Strategy Suggested by Instructor
...
* **ASC** → Increasing / Alphabetical order
* **DESC** → Decreasing / Reverse order
...
### Example: Cities with ≥ 2 Students
...
## 12. Instructor’s Advice (Key Exam Tip)

> **Don’t memorize — practice and observe output.**
...
With `GROUP BY` → works per group
Without `GROUP BY` → works on whole table
...
> Find all students who belong to **Ajay’s department**

Ajay’s department = 10
Output → Vijay, Michael, Suraj, Sharma
...
> Find students whose roll number is **greater than Suraj’s roll number**

Suraj’s roll number = 11
Output → Roll numbers 12–15
...
1. Inner query finds average roll number (≈ 8)
...
> Practice → Observation → Understanding
...
### 1.1 What Problem Does JDBC Solve?
...
### 1.2 High-Level JDBC Working Flow
...
This flow is universal across programming languages—not only Java.


### 1.3 Core Components of a JDBC Program
...
* Converts **Java data types ↔ Database data types**
...
* Represents a **live communication channel**
...
### 1.4 JDBC Execution Lifecycle (Step-by-Step)
...
## 2. JDBC Driver Types (Most Important Exam Topic)
...
* Every language + every database would otherwise need a separate driver
* ODBC acts as a **neutral middle layer**

#### Characteristics

* Part Java, part native (Windows-based)
...
* SQLite → native SQLite libraries
...
Introduces a **middleware server**
...
### 2.4 Type-4 Driver: Thin Driver (Best & Most Used)
...
> 🔑 **Order matters** — skipping or rearranging steps causes runtime errors.


## 1.3 Step 1: Downloading the SQLite JDBC Driver
...
## 1.4 Step 2: Setting the Classpath
...
2. Create:
   ...
   ```
   CLASSPATH = C:\MyJava\sqlite-jdbc-3.30.1.jar
   ```

> If the classpath is incorrect, `ClassNotFoundException` will occur.


## 1.5 Step 3: Writing the JDBC Program

### 1.5.1 Import JDBC Classes
...
### 1.5.3 Establish Database Connection
...
### 1.5.7 Exception Handling (Simplified)
...
## 1.6 Complete JDBC Program (Clean Version)
...
## 1.7 Common Errors & Their Meaning
...
# JDBC: DriverManager, Connection, and ResultSet
...
## 2. DriverManager: Connect Java to Database
...
* **DriverManager Methods**
  ...
  3. `getConnection(String url, Properties info)` – optional properties for connection
...
1. **Exceptions**
   ...
   * `ClassNotFoundException` → if JDBC driver is missing
   * `SQLException` → if database or table is not found
...
1. **Statement** – simple queries (SELECT, etc.)
2. **PreparedStatement** – queries with parameters (dynamic input)
3. **CallableStatement** – calls stored procedures in the database
...
* **Methods:**
  ...
  * `executeUpdate(String sql)` – for DML queries (`INSERT`, `UPDATE`, `DELETE`)
  * `execute(String sql)` – generic execution (returns boolean)
...
We’ll focus on **SQLite** database (`univ` or `Unif`) and its tables: **department (DPT)** and **students**.
...
* **Important notes:**
  ...
  * Integer values don’t need quotes.
